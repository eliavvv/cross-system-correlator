...
class ParseReport:
    ...
    def __init__(self):
        """Initialize error list and counts."""
...
def parse_api(path: str, report: ParseReport) -> List[Event]:
    ...
    events: List[Event] = []
    pat = re.compile(
        r"^(?P<ts>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) \| (?P<user>[^|]+) \| (?P<method>[A-Z]+) (?P<path>\S+) \| request_id=(?P<rid>\S+)$"
    )
...
def parse_db(
    path: str, report: ParseReport, assumed_date: Optional[str] = None
) -> List[Event]:
    """
    ...
    Args:
      path: str:
      report: ParseReport:
      assumed_date: Optional[str]:  (Default value = None)
      path: str:
      report: ParseReport:
      assumed_date: Optional[str]:  (Default value = None)
      path: str:
    ...
    events: List[Event] = []
    pat = re.compile(
        r'^\[(?P<hms>\d{2}:\d{2}:\d{2})\]\s+user=(?P<user>\w+)\s+query="(?P<query>.+?)"(?:\s+triggered_by=(?P<rid>\S+))?(?:\s+rows=(?P<rows>-?\d+))?$'
    )
    base_date = None
    if assumed_date:
        ...
        for i, line in enumerate(f, 1):
            ...
            if rows is not None:
                try:
                    ...
                    meta["rows"] = rows
            events.append(
                Event(ts=ts, source="DB", actor=user, action=action, meta=meta)
            )
            report.counts["DB"] += 1
    return events
    ...
    with open(path, "r", encoding="utf-8") as f:
        for i, line in enumerate(f, 1):
            ...
            try:
                ...
                events.append(
                    Event(
                        ts=ts, source="STORAGE", actor=actor, action=action, meta=meta
                    )
                )
