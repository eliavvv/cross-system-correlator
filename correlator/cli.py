...
from .parsers import parse_api, parse_db, parse_storage, ParseReport
from .correlate import build_chains, format_text, format_json

def main(argv=None):
    parser = argparse.ArgumentParser(prog="correlator", description="Cross-System Event Correlator")
    ...
    c.add_argument("--date", required=False, help="Assumed date for DB times, e.g., 2025-09-30")
    c.add_argument("--window", type=int, default=60, help="Correlation window in seconds")
    c.add_argument("--format", choices=["text","json"], default="text", help="Output format")
    c.add_argument("--show-orphans", action="store_true", help="Also print orphans and parse errors")
        ...
        if args.format == "text":
            ...
            if args.show_orphans:
                ...
                for e in sorted(orphans, key=lambda x: x.ts):
                    print(f"{e.ts.isoformat()} {e.source} {e.actor} :: {e.action} :: {e.meta}")
                if report.errors:
                    print("\n=== PARSE ERRORS ===")
            ...
            if args.show_orphans:
                obj["orphans"] = [{
                    "timestamp": e.ts.isoformat(), "source": e.source, "actor": e.actor, "action": e.action, "meta": e.meta
                } for e in orphans]
